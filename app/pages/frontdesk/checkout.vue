...
<template>
    <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6">
        ...
        <div v-if="reservation && guest && folio" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div v-if="charges.length > 0" class="space-y-2">
                                <div v-for="c in charges" :key="c.id"
                                    class="flex justify-between text-sm py-1 border-b border-default border-dashed">
                        ...
                        <div>
                            ...
                            <div v-if="payments.length > 0" class="space-y-2">
                                <div v-for="p in payments" :key="p.id"
                                    class="flex justify-between text-sm py-1 border-b border-default border-dashed text-success-600">
            ...
            <!-- Right Column: Action -->
            <div class="space-y-6">
                <UCard class="border-2"
                    :class="canCheckout ? 'border-success-200 dark:border-success-800 bg-success-50 dark:bg-success-950/20' : 'border-error-200 dark:border-error-800 bg-error-50 dark:bg-error-950/20'">

                    <div class="text-center mb-6">
                        <UIcon :name="canCheckout ? 'i-lucide-check-circle-2' : 'i-lucide-alert-circle'"
                            class="size-16 mb-2" :class="canCheckout ? 'text-success-500' : 'text-error-500'" />
                        <h3 class="font-bold text-lg"
                            :class="canCheckout ? 'text-success-900 dark:text-success-100' : 'text-error-900 dark:text-error-100'">
                    ...
                    <div v-if="!canCheckout" class="text-sm text-center text-error-700 dark:text-error-300 mb-6">
                        Guest has an outstanding balance of <strong>{{ formatCurrency(folio.balance) }}</strong>.
                        <br /><br />
                    ...
                    <UButton v-if="!canCheckout" label="Go to Billing Module" color="error" variant="soft" size="lg"
                        block icon="i-lucide-external-link" @click="router.push('/billing')" />

                    <UButton v-else label="Confirm Check-Out" color="success" size="xl" block icon="i-lucide-log-out"
