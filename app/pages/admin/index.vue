...
<template>
    <AuthGate v-if="!isAuthorized" title="Access Denied"
        description="You must be an Administrator to access the Admin Dashboard." icon="i-lucide-lock" />
    ...
    <template v-else>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mt-6">
            ...
            <StatCard title="Arrivals Today" :value="reservationsStore.arrivalsToday.length" icon="i-lucide-log-in"
                :trend="`${reservationsStore.departuresToday.length} departures`" trend-direction="flat" />
            <StatCard title="Open Balance" :value="formatCurrency(openBalance)" icon="i-lucide-wallet"
                :trend="`${foliosStore.openFolios.length} open folios`" trend-direction="flat" />
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mt-6">
            ...
            <UCard variant="subtle" class="shadow-sm">
                ...
                <div class="h-[260px] w-full">
                    <Bar v-if="reservationsStore.reservations.length > 0" :data="reservationStatusData"
                        :options="defaultOptions" />
                    <p v-else class="text-sm text-muted flex items-center justify-center h-full">No reservation data.
                        Deploy demo data to see chart.</p>
                </div>
            </UCard>
        </div>
